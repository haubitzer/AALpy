class IocoValidator:
    ...
    def _resolve_state(self, original_state: IoltsState, test_state: IoltsState):
        ...
        for input in self.specification.get_input_alphabet():
            ...
            k_prime.update(self._resolve_inputs(original_state, test_state, destinations, input))

        for output in self.specification.get_output_alphabet():
            destinations = [state for _, state in original_state.get_outputs(output)]
            k_prime.update(self._resolve_outputs(original_state, test_state, destinations, output))

        for new_test_state, k_list in k_prime.items():
            for state in k_list:
                for output in self.specification.get_output_alphabet():
                    destinations = [state for _, state in state.get_outputs(output)]
                    k_alpha.update(self._resolve_outputs(original_state, new_test_state, destinations, output))

                destinations = [state for _, state in state.get_quiescence()]
                k_alpha.update(self._resolve_quiescence(state, new_test_state, destinations))
    ...
    def _resolve_inputs(self, _original_state: IoltsState, test_state: IoltsState, destinations: list[IoltsState],
                        letter: str) -> dict:
        follow_state = dict()
    ...
    def _resolve_outputs(self, _original_state: IoltsState, test_state: IoltsState, destinations: list[IoltsState],
                         letter: str) -> dict:
        follow_state = dict()
    ...
    def _resolve_quiescence(self, state: IoltsState, test_state: IoltsState,
    ...
    def _resolve_non_deterministic(self, sut: IoltsMachine, current_state_id: str, path: list, resolved_path: list,
        ...
        resolved_path_list = [self._resolve_non_deterministic(sut, current_state_id, path.copy(), resolved_path.copy(),
